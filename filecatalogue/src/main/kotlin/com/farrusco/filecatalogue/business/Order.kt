...
class Order(context: Context) : TableOrder(context){

    fun getOrder(id: Int): ReturnValue {
        val map = ContentValues()
        if (id == 0) {
            map.put("orderby","${Columns.orderdate.name} desc, ${Columns._id.name}")
        } else {
            map.put(Columns._id.name,id)
        }
        return super.selectContentValues(map)
    }
