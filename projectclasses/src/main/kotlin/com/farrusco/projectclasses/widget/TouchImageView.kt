...
import com.farrusco.projectclasses.common.ZoomVariables
import com.farrusco.projectclasses.widget.touchview.OnTouchCoordinatesListener
import com.farrusco.projectclasses.widget.touchview.OnTouchImageViewListener
...
open class TouchImageView @JvmOverloads
constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) :
    ...
    private var orientationChangeFixedPixel: ConstantsFixed.FixedPixel? = ConstantsFixed.FixedPixel.CENTER
    private var viewSizeChangeFixedPixel: ConstantsFixed.FixedPixel? = ConstantsFixed.FixedPixel.CENTER
    private var orientationJustChanged = false

    private var imageActionState: ConstantsFixed.ImageActionState? = null
    ...
    //private var isImageFitToScreen = true
    ...
    init {
        ...
        setState(ConstantsFixed.ImageActionState.NONE)
        onDrawReady = false
...
/* test      if (bm!=null){
            ...
            p.color = Color.RED
            p.strokeWidth=2f
            canvas.drawLine(0f,0f,0f,bm.height.toFloat(),p)
            canvas.drawLine(0f,0f,bm.width.toFloat(),0f,p)
            canvas.drawLine(bm.width.toFloat(),bm.height.toFloat(),0f,bm.height.toFloat(),p)
            canvas.drawLine(bm.width.toFloat(),bm.height.toFloat(),bm.width.toFloat(),0f,p)
        }*/
    ...
    private fun runObjectDetection(bitmap: Bitmap) {
        ...
        } catch(e: AbstractMethodError) {} catch(e: Exception) {}
    ...
    private fun debugPrint(detectedObjects: List<DetectedObject>) {
        detectedObjects.forEachIndexed { index, detectedObject ->
            ...
            Logging.d( "Detected object: $index")
            Logging.d(  " trackingId: ${detectedObject.trackingId}")
            Logging.d(  " boundingBox: (${box.left}, ${box.top}) - (${box.right},${box.bottom})")
            detectedObject.labels.forEach {
                Logging.d( " categories: ${it.text}")
                Logging.d(  " confidence: ${it.confidence}")
            }
        }
    }
    */
    ...
    public override fun onRestoreInstanceState(state: Parcelable) {
        if (state is Bundle) {
            ...
            viewSizeChangeFixedPixel = state.serializable("viewSizeChangeFixedPixel") as ConstantsFixed.FixedPixel?
            orientationChangeFixedPixel = state.serializable("orientationChangeFixedPixel") as ConstantsFixed.FixedPixel?
    ...
    private fun newTranslationAfterChange(
        ...
        viewSize: Int,
        drawableSize: Int,
        sizeChangeFixedPixel: ConstantsFixed.FixedPixel?
    ): Float {
        return when {
            imageSize < viewSize -> {
            ...
            trans > 0 -> {
                ...
                if (sizeChangeFixedPixel == ConstantsFixed.FixedPixel.BOTTOM_RIGHT) {
                    fixedPixelPositionInView = 1.0f
                } else if (sizeChangeFixedPixel == ConstantsFixed.FixedPixel.TOP_LEFT) {
    ...
    private fun setState(imageActionState: ConstantsFixed.ImageActionState) {
    ...
    private inner class GestureListener : SimpleOnGestureListener() {
        ...
        override fun onDoubleTap(e: MotionEvent): Boolean {
            ...
            if (isZoomEnabled) {
                ...
                if (imageActionState == ConstantsFixed.ImageActionState.NONE) {
    ...
    private inner class PrivateOnTouchListener : OnTouchListener {
        ...
        override fun onTouch(v: View, event: MotionEvent): Boolean {
            ...
            if (imageActionState == ConstantsFixed.ImageActionState.NONE || imageActionState == ConstantsFixed.ImageActionState.DRAG || imageActionState == ConstantsFixed.ImageActionState.FLING) {
                when (event.action) {
                    MotionEvent.ACTION_DOWN -> {
                        last.set(curr)
                        fling?.cancelFling()
                        setState(ConstantsFixed.ImageActionState.DRAG)

                    }
                    MotionEvent.ACTION_MOVE -> if (imageActionState == ConstantsFixed.ImageActionState.DRAG) {
                    ...
                    MotionEvent.ACTION_UP, MotionEvent.ACTION_POINTER_UP -> setState(ConstantsFixed.ImageActionState.NONE)
                }
            }
    ...
    */
    ...
    private inner class ScaleListener : SimpleOnScaleGestureListener() {
        override fun onScaleBegin(detector: ScaleGestureDetector): Boolean {
            setState(ConstantsFixed.ImageActionState.ZOOM)
            return true
        }
    ...
    private inner class DoubleTapZoom(targetZoom: Float, focusX: Float, focusY: Float, stretchImageToSuper: Boolean) : Runnable {
        ...
        init {
            setState(ConstantsFixed.ImageActionState.ANIMATE_ZOOM)
    ...
    private inner class Fling(velocityX: Int, velocityY: Int) : Runnable {
        ...
        init {
            setState(ConstantsFixed.ImageActionState.FLING)
