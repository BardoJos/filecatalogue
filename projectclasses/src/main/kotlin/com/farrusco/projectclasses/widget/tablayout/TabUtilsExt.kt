...
internal object TabUtilsExt {
    ...
    fun getStart(v: View?, withoutPadding: Boolean): Int {
        ...
        return if (isLayoutRtl(v)) {
            if (withoutPadding) v.right - getPaddingStart(
                v
            ) else v.right
        } else {
            if (withoutPadding) v.left + getPaddingStart(
                v
            ) else v.left
        }
    }
    ...
    fun getEnd(v: View?, withoutPadding: Boolean): Int {
        ...
        return if (isLayoutRtl(v)) {
            if (withoutPadding) v.left + getPaddingEnd(
                v
            ) else v.left
        } else {
            if (withoutPadding) v.right - getPaddingEnd(
                v
            ) else v.right
        }
    }
    ...
    fun getGravity(v: View?): Int {
        return (v as TabLayout).tabGravity
    }
