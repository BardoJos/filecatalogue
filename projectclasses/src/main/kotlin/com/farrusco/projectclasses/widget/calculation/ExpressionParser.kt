...
class ExpressionParser {
    ...
    fun evaluate(expression: String, precision: Int = 3): Double {
        val uExpression = convertToUExpression(expression.replace(" ",""))
    ...
    private fun evaluateExpression(expression: String): Double {
        ...
        while (i < expression.length) {
            ...
            } else if (currChar.toString() isIn NormalOperators.entries.toTypedArray() || currChar == '(') {

                if (currChar == '(') {
                    // check for implicit multiply
                    if (i != 0 && expression[i - 1].toString() notIn NormalOperators.entries.toTypedArray()) {
                        performSafePushToStack(numString, "*")
                    }
                    opStack.push("(")
                } else {
        ...
        while (!opStack.isEmpty()) {
            val op = opStack.pop()
            if (op isIn FunctionalOperators.entries.toTypedArray()) {
                clearStacks()
                throw BadSyntaxException()
            }
    ...
    private fun pushFunctionalOperator(
        ...
        for (func in FunctionalOperators.entries) {
    ...
    private fun computeBracket(numString: StringBuilder) {
        ...
        while (operator != "(" && operator notIn FunctionalOperators.entries.toTypedArray()) {
            computeNormalOperation(operator)
            operator = opStack.pop()
        }
        if (operator isIn FunctionalOperators.entries.toTypedArray()) {
    ...
    private fun Double.isInt() = ceil(this) == floor(this)
