...
internal object BitmapUtils {
   ...
   * New bitmap is created and the old one is recycled.
   */
  private fun orientateBitmapByExif(bitmap: Bitmap?, exif: ExifInterface): RotateBitmapResult {
  ...
  fun writeBitmapToUri(
    ...
    val newUri = customOutputUri ?: buildUri(compressFormat)
    ...
    try {
      ...
      if (SDK_INT >= 29) {
        try {
          ...
          Log.e("AIC", "${e.message}")
          val file = File.createTempFile("cropped", ext, dir)
          //getUriForFile(context, file)
          file.toUri()
        }
      } else {
        Uri.fromFile(File.createTempFile("cropped", ext, dir))
      }
    } catch (e: IOException) {
  ...
  fun resizeBitmap(
    ...
    try {
      if (reqWidth > 0 && reqHeight > 0 && (options === CropImageView.RequestSizeOptions.RESIZE_FIT
                || options === CropImageView.RequestSizeOptions.RESIZE_INSIDE || options === CropImageView.RequestSizeOptions.RESIZE_EXACT)) {
        var resized: Bitmap? = null
        if (options === CropImageView.RequestSizeOptions.RESIZE_EXACT) {
          resized = bitmap!!.scale(reqWidth, reqHeight, false)
        } else {
          ...
          if (scale > 1 || options === CropImageView.RequestSizeOptions.RESIZE_FIT) {
            resized = bitmap.scale((width / scale).toInt(), (height / scale).toInt(), false)
          }
        }
        if (resized != null) {
          if (resized != bitmap) {
